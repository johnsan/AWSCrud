...
public class SnowboardDAOImpl implements SnowboardDAO {
	...
	static {
		properties.put("lobBucketName",
				S3StorageManager.getKey().toLowerCase() + "-travellog-lob"
						+ StageUtils.getResourceSuffixForCurrentStage());
	}

	private static EntityManagerFactoryImpl factory = new EntityManagerFactoryImpl(
			"TravelLog" + StageUtils.getResourceSuffixForCurrentStage(),
			properties);

	private static final Logger log = Logger.getLogger(SnowboardDAOImpl.class
			.getName());

	public boolean createSnowboard(Snowboard snowboard) throws Exception {

		log.info("Creating snowboard... " + "Brand: " + snowboard.getBrand()
				+ " " + "Model: " + snowboard.getModel() + " " + "Length: "
				+ snowboard.getLength() + " " + "Genre: "
				+ snowboard.getGenre());
	...
	public List<Snowboard> getAllSnowboards() throws Exception {
		...
		try {
			em = factory.createEntityManager();
			Query query = em
					.createQuery("select s from com.awscrud.domain.Snowboard s");
	...
	public void deleteSnowboardById(long snowboardId) throws Exception {
		...
		try {
			em = factory.createEntityManager();
			Query query = em
					.createQuery("select s from com.awscrud.domain.Snowboard s where s.id= :snowboardid");
	...
	public boolean updateSnowboard(Long snowboardId, String fieldModified,
		...
		log.info("Updating snowboard with id: " + snowboardId);
	 ...
	 * em.persist(snowboard); em.close();
	 * 
	 * }
	 * 
	 * public Snowboard getSnowboardById(int snowboardId) throws Exception {
	 * EntityManager em = SimpleDBEntityManagerFactory.getEntityManager(); Query
	 * q = em.createQuery("select o from Snowboard o where o.id= :snowboardid");
	 * q.setParameter("snowboardid", snowboardId); List<Snowboard> obs =
	 * q.getResultList(); Snowboard snowboard = (Snowboard) obs.get(0);
	 * em.close(); return snowboard; }
	 * 
	 * public void deleteSnowboard(Snowboard snowboard) throws Exception {
	 * EntityManager em = SimpleDBEntityManagerFactory.getEntityManager();
	 * em.remove(snowboard); em.close(); }
	 * 
	 * public int getTotalCountOfSnowboards() throws Exception { EntityManager
	 * em = SimpleDBEntityManagerFactory.getEntityManager(); Query q =
	 * em.createQuery("select o from Snowboard o"); List<Snowboard> obs =
	 * q.getResultList(); em.close(); return obs.size(); }
	 */
